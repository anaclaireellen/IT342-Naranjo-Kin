...
import AestheticChat from '../messaging/AestheticChat';
import AdminPanel from './AdminPanel';
import { clearStoredSession, getStoredProfile, PROFILE_UPDATED_EVENT } from '../profile/profileHelpers';
...
const Dashboard = ({ setAuth }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: isCompactLayout ? 'column' : 'row', height: isCompactLayout ? 'auto' : 'calc(100vh - 36px)', minHeight: isCompactLayout ? '100vh' : 0, background: appTheme.background, fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', padding: isCompactLayout ? '12px' : '18px', gap: '18px', boxSizing: 'border-box', overflow: isCompactLayout ? 'visible' : 'hidden' }}>
      <aside className="kin-animate-in" style={{ width: isCompactLayout ? '100%' : '292px', background: '#ffffff', border: `1px solid ${appTheme.border}`, display: 'flex', flexDirection: 'column', padding: '18px', flexShrink: 0, borderRadius: '24px', boxShadow: appTheme.shadow }}>
        <div style={{ padding: '12px 12px 18px', borderBottom: `1px solid ${appTheme.border}` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', marginBottom: '18px' }}>
          <div style={{ width: '54px', height: '54px', borderRadius: '16px', background: appTheme.cardAlt, border: `1px solid ${appTheme.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ padding: '6px 10px', borderRadius: '999px', background: 'rgba(49,214,200,0.22)', border: '1px solid rgba(49,214,200,0.34)', fontSize: '10px', letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: '700', color: appTheme.primaryStrong }}>
              Student
            </div>
          </div>
        ...
        <div className="kin-interactive-card" onClick={() => setActiveTab('settings')} style={{ marginTop: '10px', background: 'linear-gradient(135deg, rgba(15,117,128,0.16) 0%, rgba(49,214,200,0.24) 100%)', padding: '14px', borderRadius: '18px', border: `1px solid ${appTheme.border}`, cursor: 'pointer' }}>
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, minHeight: isCompactLayout ? '560px' : 0, overflow: 'hidden' }}>
        {activeTab === 'messages' ? (
          <AestheticChat
          ...
          <div className="kin-scrollbar kin-animate-in" style={{ flex: 1, overflow: 'auto', background: appTheme.card, borderRadius: '24px', border: `1px solid ${appTheme.border}`, boxShadow: appTheme.shadow, minHeight: 0 }}>
